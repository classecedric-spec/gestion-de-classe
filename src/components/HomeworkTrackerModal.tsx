...
interface HomeworkTrackerModalProps {
    isOpen: boolean;
    onClose: () => void;
    students: any[];
}

const HomeworkTrackerModal: React.FC<HomeworkTrackerModalProps> = ({ isOpen, onClose, students }) => {
    const [date, setDate] = useState(new Date().toISOString().split('T')[0]);
    const [statusMap, setStatusMap] = useState<Record<string, string>>({}); // { studentId: 'fait' | 'non_fait' | 'rattrape' }
            ...
            <div className="bg-surface border border-white/10 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="p-4 border-b border-white/5 flex items-center gap-4 bg-surface">
                    <input
                        ...
                        onClick={(e) => { try { (e.target as any).showPicker(); } catch (err) { /* Silently fail if showPicker not supported */ } }}
                        onFocus={(e) => { try { (e.target as any).showPicker(); } catch (err) { /* Silently fail if showPicker not supported */ } }}
                                    ...
                                    >
                                        <div className="w-10 h-10 rounded-full bg-background border border-white/10 overflow-hidden flex items-center justify-center shrink-0">
                                            {(student.photo_url || student.photo_base64) ? (
                                                <img src={student.photo_url || student.photo_base64 || ''} className="w-full h-full object-cover" />
                                            ) : (
