...
import { supabase } from '../../../lib/supabaseClient'; // Keep generic supabase client for auth check if needed, or move auth to service
...
const AttendanceConfigModal = ({
    ...
    // attendancesForExport = [], // For PDF (Default for "Day" mode initially)
    ...
    const fetchAttendanceRange = useCallback(async (start, end) => {
        if (!selectedGroup) return;
        setLoading(true);
        try {
            const data = await attendanceService.fetchAttendanceRange(
                start,
                end
            );
            setExportData(data || []);
    ...
    useEffect(() => {
        if (activeTab === 'export' && selectedGroup && selectedSetup) {
            fetchDistinctDates().then((dates) => {
                if (exportMode !== 'day' && dates) {
                    // logic handled in dependency chain usually but we need to ensure local dates access
                    // handled below in separate effect
                }
            });
            setExportMode('day');
    ...
    useEffect(() => {
        if (activeTab === 'export' && exportMode !== 'day' && distinctDates.length > 0) {
            // Process periods logic moved inline or called here
            const processAvailablePeriods = (uniqueDates, mode) => {
                ...
                uniqueDates.forEach(dateStr => {
                    ...
                    let value, label; // removed start/end unused vars
    ...
    useEffect(() => {
        ...
        if (exportMode === 'day') {
            ...
            // Generate full range of dates (local timezone to avoid day-of-week issues)
            const dates = [];
    ...
    const handleEdit = async (set) => {
        ...
        setLoading(true);
        try {
            // Fetch categories for this set, exclude Absent as it's system managed usually or handled separately
            const data = await attendanceService.fetchCategories(set.id);
            // Filter out 'Absent' from edit view to prevent accidental modification if desired, or keep it.
            // keeping behavior of original code: .neq('nom', 'Absent')
    ...
    return (
        ...
        >
            ...
            {/* Sticky Footer - Copy Buttons (only visible in General tab) */}
            {activeTab === 'general' && (
