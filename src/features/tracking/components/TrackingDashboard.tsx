...
// Hooks
import { useTimerIntegration, Timer } from '../hooks/useTimerIntegration';
                                            ...
                                            <div className="flex flex-col gap-3 animate-in fade-in duration-500">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-primary/20 shadow-lg shrink-0 bg-surface">
                                                        {(groupsHook.states.selectedStudent as any).photo_base64 ? (
                                                            <img src={(groupsHook.states.selectedStudent as any).photo_base64} alt="" className="w-full h-full object-cover" />
                                                        ) : (
                                ...
                                currentView === 'modules' && (
                                    branchesHook.states.loadingModules ? (
                                        ...
                                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300 pb-10">
                                            {branchesHook.states.modules.map((module: any) => {
                                                        ...
                                                        {isExpanded && (
                                                            <div className="px-3 pb-4 space-y-2.5 animate-in slide-in-from-top-2 duration-300 border-t border-white/5 pt-3">
                                                                {branchesHook.states.loadingActivities ? (
                                                                    ...
                                                                    branchesHook.states.activities.map((activity: any) => (
                                                                        <ProgressionCell
                                                                            key={activity.id}
                                                                            activity={activity}
                                                                            currentStatus={normalizeStatus(progressionsHook.states.progressions[activity.id]) as ProgressionStatus}
