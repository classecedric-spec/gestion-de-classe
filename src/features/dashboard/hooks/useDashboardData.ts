...
export const useDashboardData = (userId?: string, allStudents: Student[] = []) => {
    ...
    const { data: dashboardData = initialDashboardData, isLoading: loading } = useQuery({
        ...
        queryFn: async (): Promise<DashboardData> => {
            ...
            (branchData as any[])?.forEach(p => {
                ...
                if (!branchCounts[bNom]) branchCounts[bNom] = { total: 0, done: 0, byLevel: {} };
            ...
            const branchStats: BranchStat[] = Object.entries(branchCounts).map(([name, stats]) => {
            ...
            const leastActiveYesterday = allStudents.map(s => ({
                ...s,
            ...
            const studentBreakdownRaw: Record<string, any> = allStudents.reduce((acc, student) => {
                ...
                if (!acc[levelName]) acc[levelName] = { name: levelName, order: levelOrder, total: 0, boys: 0, girls: 0 };
                acc[levelName].total++;
            ...
            rawLateActivities.forEach(item => {
                ...
                if (!overdueByLevel[levelKey]) overdueByLevel[levelKey] = { name: levelKey, ordre: level.ordre, students: {} };
                if (!overdueByLevel[levelKey].students[student.id]) overdueByLevel[levelKey].students[student.id] = { ...student, overdueModules: {} };
                const module = item.Activite?.Module;
                if (!module) return;
                if (!overdueByLevel[levelKey].students[student.id].overdueModules[module.id]) overdueByLevel[levelKey].students[student.id].overdueModules[module.id] = { ...module, activities: [] };
                overdueByLevel[levelKey].students[student.id].overdueModules[module.id].activities.push({ id: item.id, titre: item.Activite?.titre, etat: item.etat });
            });

            const finalOverdueData = Object.values(overdueByLevel).sort((a: any, b: any) => a.ordre - b.ordre).map((level: any) => ({
                ...level,
                students: Object.values(level.students).map((student: any) => ({
                    ...student,
                    overdueModules: Object.values(student.overdueModules),
                    overdueCount: Object.values(student.overdueModules).reduce((sum: number, mod: any) => sum + mod.activities.length, 0)
                }))
            }));
            ...
            return {
                stats: {
                    ...
                    studentBreakdown,
                    attendance: { todayCount: presentCount, monthlyAvg: averagePresence },
                    progression: { rate: completionRate, todayValidations, todayRate: todayValidationRate, doneCount, totalStarted },
                    monthlyActions: monthlyData?.length || 0
                },
                birthdays,
                nextMonthBirthdays,
                recentActivity: recentActivity || [],
                priorityStudents: { yesterday: leastActiveYesterday, week: leastValidatedWeek, completion: lowestCompletionRate, lastActiveLabel: lastActiveDayLabel },
                ...
                dailySummary: {
                    ...
                    helpNeeded: allStudents.filter(s => progressionAll?.some(p => p.eleve_id === s.id && p.updated_at?.startsWith(today) && p.etat === 'besoin_d_aide')),
                    quietStudents: allStudents.filter(s => !progressionAll?.some(p => p.eleve_id === s.id && p.updated_at?.startsWith(today))).slice(0, 5),
                    domainFocus: branchStats.slice(0, 3)
                },
                planning: { count: plannedActivitiesCount || 0, label: weekLabel },
                overdueStudents: finalOverdueData
            };
        },
        enabled: !!userId && allStudents.length > 0,
    ...
    return {
        dashboardData,
        loading,
        fetchDashboardDetails: () => queryClient.invalidateQueries({ queryKey: ['dashboard-stats', userId] }),
        LEVEL_COLORS
    };
};
