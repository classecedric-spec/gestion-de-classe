...
export const useDashboardData = () => {
    ...
    const fetchDashboardDetails = useCallback(async (userId, allStudents) => {
        ...
        try {
            ...
            if (daysWithAttendance.length > 0) {
                const totalPresentsMonth = attendanceData?.filter(a => {
                    if (a.status === 'present') return true;
                    if (a.CategoriePresence && a.CategoriePresence.nom !== 'Absent') return true;
                    return false;
                }).length || 0;
