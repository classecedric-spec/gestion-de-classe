...
import {
    ...
    Key, Feather, Smartphone, Cpu, Image, Activity, Layers, ArrowRight, ChevronDown
} from 'lucide-react';
...
const Settings = () => {
    ...
    const [selectedBulkIndex, setSelectedBulkIndex] = useState(50);
    ...
    const handleBulkUpdateIndices = async () => {
        if (!selectedBulkBranch) {
            toast.error("Veuillez sélectionner une branche");
            return;
        }

        const confirmMsg = `Voulez-vous vraiment appliquer l'indice ${selectedBulkIndex}% à TOUS les élèves (${allStudents.length}) pour la branche sélectionnée ?`;
        if (!confirm(confirmMsg)) return;

        setIsUpdatingBulk(true);
        const toastId = toast.loading("Mise à jour des indices en cours...");
        ...
        try {
            ...
            const currentPrefs = prefData?.value || {};
            const newPrefs = { ...currentPrefs };

            // 2. Update all students for the selected branch
            allStudents.forEach(student => {
                if (!newPrefs[student.id]) {
                    newPrefs[student.id] = {};
                }
                newPrefs[student.id][selectedBulkBranch] = parseInt(selectedBulkIndex);
            });
            ...
            console.error("Error bulk updating indices:", err);
    ...
    const handleBulkAdjustIndices = async (delta) => {
        if (!selectedBulkBranch) {
            toast.error("Veuillez sélectionner une branche");
            return;
        }
        ...
        const confirmMsg = `Voulez-vous vraiment ${action} l'indice de 10% pour TOUS les élèves (${allStudents.length}) sur la branche sélectionnée ?`;
        if (!confirm(confirmMsg)) return;
        ...
        try {
            ...
            toast.success(`Indices ajustés de ${delta > 0 ? '+' : ''}${delta}% pour ${allStudents.length} élèves !`, { id: toastId });
        } catch (err) {
            console.error("Error adjusting indices:", err);
                                        ...
                                        <div className="flex flex-col md:flex-row items-center justify-between pt-4 border-t border-white/5 gap-4">
                                            <div className="flex-1 space-y-1">
                                                <p className="text-[11px] text-grey-medium leading-tight max-w-md">
                                                    <span className="text-warning font-bold">Attention:</span> Cette action modifiera l'indice de performance pour <span className="text-white font-bold">{allStudents.length} élèves</span> sur la branche sélectionnée.
                                                </p>
                                                <p className="text-[10px] text-grey-dark italic">
                                                    Utile pour ajuster la fréquence de vérification globale.
                                                </p>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2 w-full md:w-auto">
                                                <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 gap-1">
                                                    <button
                                                        ...
                                                        title="Diminuer l'indice de 10% pour tous"
                                                    >
                                                        -10%
                                                    </button>
                                                    <button
                                                        ...
                                                        disabled={isUpdatingBulk || isLoadingBulkData || !selectedBulkBranch}
                                                        className="px-4 py-2 hover:bg-white/10 text-white rounded-lg transition-all text-[11px] font-black disabled:opacity-30"
                                                        title="Augmenter l'indice de 10% pour tous"
                                                    >
                                                        +10%
                                                    </button>
                                                </div>
