...
import { Settings, Lock, Unlock, Calendar, Download } from 'lucide-react';
...
const Presence = () => {
    const {
        ...
        // getUnassignedStudents, // used trulyUnassigned instead
        // getSystemAbsentStudents // not used directly
    } = useAttendance();
    ...
    const handleDragEnd = async (event) => {
        ...
        const targetId = over.id; // This is the Column ID (category ID or 'unassigned' or 'absent')

        if (targetId === 'absent') {
            // Logic for dropping into absent column specific manually?
            // Usually absent is just a status, but if we have a visual column for it:
            // "System Absent" usually doesn't have a category ID.
            // But we might map it to a specific category if one exists, or just null category + status absent.
            // For now, let's treat 'absent' drop as a specific action if we implement an absent column.
            // BUT, the hook `moveStudent` expects a CATEGORY ID.
            // If the user drags to "Absent" column, we probably want to set status='absent' and category=null (or special absent category).

            // Checking how `moveStudent` works in hook:
            // It takes `targetId`. If targetId is 'unassigned', it deletes record/sets status?
            // No, the hook `moveStudent` uses `targetId` as `categorie_id`.
            // If we want to mark as Absent via Drag, we need to pass a special ID or handle it.
            // The original code usually had an 'Absent' category in the DB.
            // Let's assume there is an 'Absent' category or we find it.
                ...
                >
                    <div className="flex-1 flex gap-6 min-h-0 overflow-hidden">
                        ...
                        <div className="w-1/4 min-w-[280px] flex flex-col gap-4">
                            ...
                            {/* Absent Column */}
                            {/* Note: In original, 'Absent' is just a category. But often we want it separate. 
                                 If 'Absent' is in categories, it renders in the grid. 
                                 Let's keep it in the grid if it exists there, but if we want strictly separated:
                                 We can filter 'Absent' out of categories.map and render it here.
                             */}
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar rounded-2xl p-1">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 pb-20">
                                {categories.map(cat => {
                                    ...
                                    // Also include "Absents" if this category is named Absent
                                    // (Our logic in getStudentsForCategory handles pure 'categorical' match)
