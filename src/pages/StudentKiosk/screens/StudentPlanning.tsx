...
import { useStudentPlanningData, PlanningActivity, PlanningStatus, PlanningChoice } from '../hooks/useStudentPlanningData';

const JOURS = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi'];
const JOURS_COURTS = ['L', 'Ma', 'Me', 'J', 'V'];

type Step = 'status' | 'homework' | 'weekwork' | 'summary' | 'done';

// --- Composant cases de statut ---
const StatusGrid: React.FC<{
    activity: PlanningActivity;
    choices: Record<string, PlanningChoice>;
    toggleStatut: (activityId: string, targetStatut: PlanningStatus) => void;
}> = ({ activity, choices, toggleStatut }) => {
...
const DayGrid: React.FC<{
    ...
    setChoice: (activityId: string, jour: string | null, lieu: 'classe' | 'domicile' | null) => void;
    toggleLieu: (activityId: string, jour: string) => void;
}> = ({ activity, mode, choices, showValidationErrors, setChoice, toggleLieu }) => {
    const choice = choices[activity.id];
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (mode === 'homework') {
                                ...
                                else setChoice(activity.id, jour, 'classe');
                            }
                        }}
                        className={clsx(
...
const ModuleBlock: React.FC<{
    ...
    setChoice: (activityId: string, jour: string | null, lieu: 'classe' | 'domicile' | null) => void;
    toggleLieu: (activityId: string, jour: string) => void;
}> = ({ moduleNom, moduleDateFin, activities: acts, mode, isOverdue, choices, showValidationErrors, toggleStatut, setChoice, toggleLieu }) => (
    <div className={clsx(
                ...
                return (
                    ...
                    >
                        <div className="flex items-center gap-3 min-w-0">
                            {mode === 'status' && <StatusGrid activity={act} choices={choices} toggleStatut={toggleStatut} />}
                            <span className={clsx(
                        ...
                        {mode !== 'status' && <DayGrid activity={act} mode={mode as 'homework' | 'weekwork'} choices={choices} showValidationErrors={showValidationErrors} setChoice={setChoice} toggleLieu={toggleLieu} />}
                    </div>
                );
            })}
        </div>
...
const SummaryView: React.FC<{
    ...
    weekModules: any[];
    choices: Record<string, PlanningChoice>;
    overdueActivities: PlanningActivity[];
    weekActivities: PlanningActivity[];
}> = ({ overdueModules, weekModules, choices, overdueActivities, weekActivities }) => {
    ...
    Object.values(choices).forEach(c => {
        ...
        const module = allModules.find(m => m.activities.some((a: any) => a.id === act.id));
    ...
    if (step === 'done') {
        return (
            <div className="flex flex-col h-full items-center justify-center p-8 text-center space-y-8 animate-in fade-in duration-500">
                ...
                <div className="space-y-4 max-w-md">
                    <h1 className="text-4xl font-black text-white uppercase tracking-tight">C'est enregistré !</h1>
                    <p className="text-xl text-grey-medium">Ta planification de la semaine est sauvegardée. Tu peux fermer cette page.</p>
                </div>
            </div>
        );
    }

    const allHomeworkAssigned = homeworkActivities.every(a => !!choices[a.id]?.jour);
        ...
        <div className="flex flex-col h-full overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar">
                <div className="max-w-3xl mx-auto w-full pt-4">
                    {step === 'status' && (
                        <div className="animate-in slide-in-from-right-4 duration-300">
                            <div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-2xl">
                                ...
                                <p className="text-primary/60 text-sm mt-1">Prends ta farde et clique sur les carrés correspondants à ton avancement.</p>
                            </div>
                            {overdueModules.length > 0 && (
                                <div className="mb-8">
                                    ...
                                    {overdueModules.map((mod, idx) => (
                                        <ModuleBlock key={`overdue-${idx}`} moduleNom={mod.moduleNom} moduleDateFin={mod.moduleDateFin} activities={mod.activities} mode="status" isOverdue choices={choices} showValidationErrors={showValidationErrors} toggleStatut={toggleStatut} setChoice={setChoice} toggleLieu={toggleLieu} />
                                    ))}
                                </div>
                            )}
                            {weekModules.length > 0 && (
                                <div>
                                    ...
                                    {weekModules.map((mod, idx) => (
                                        <ModuleBlock key={`week-${idx}`} moduleNom={mod.moduleNom} moduleDateFin={mod.moduleDateFin} activities={mod.activities} mode="status" choices={choices} showValidationErrors={showValidationErrors} toggleStatut={toggleStatut} setChoice={setChoice} toggleLieu={toggleLieu} />
                                    ))}
                                </div>
                            )}
                        </div>
                            ...
                            {homeworkModules.length === 0 ? (
                                ...
                                <>
                                    ...
                                    {homeworkModules.map((mod) => {
                                        const filteredActs = mod.activities.filter(a => !onlyShowUnassigned || !choices[a.id]?.jour);
                                        if (filteredActs.length === 0) return null;
                                        return <ModuleBlock key={mod.moduleNom} moduleNom={mod.moduleNom} moduleDateFin={mod.moduleDateFin} activities={filteredActs} mode="homework" isOverdue choices={choices} showValidationErrors={showValidationErrors} toggleStatut={toggleStatut} setChoice={setChoice} toggleLieu={toggleLieu} />;
                                    })}
                                </>
                            )}
                        </div>
                            ...
                            {weekworkModules.length === 0 ? (
                                ...
                                <>
                                    <div className="mb-6 p-4 bg-primary/10 border border-primary/20 rounded-2xl">
                                        ...
                                        <p className="text-primary/60 text-sm mt-1">Clique sur un jour : 🟢 vert = en classe, 🟠 orange = à domicile. Reclique pour changer.</p>
                                    </div>
                                    {weekworkModules.map((mod, idx) => (
                                        <ModuleBlock key={idx} moduleNom={mod.moduleNom} moduleDateFin={mod.moduleDateFin} activities={mod.activities} mode="weekwork" isOverdue={mod.activities.some(a => a.isOverdue)} choices={choices} showValidationErrors={showValidationErrors} toggleStatut={toggleStatut} setChoice={setChoice} toggleLieu={toggleLieu} />
                                    ))}
                                </>
                            )}
                        </div>
                    ...
                    {step === 'summary' && (
                        <div className="animate-in slide-in-from-right-4 duration-300">
                            <h2 className="text-2xl font-black text-white text-center mb-6">Récapitulatif de ta semaine</h2>
                            <SummaryView overdueModules={overdueModules} weekModules={weekModules} choices={choices} overdueActivities={overdueActivities} weekActivities={weekActivities} />
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="max-w-4xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        {step !== 'status' && (
                            ...
                            }} className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-white text-sm font-bold border border-white/10 transition-all active:scale-95">← Retour</button>
                        )}
                        ...
                        {step === 'homework' && (
                            <div className="flex items-center gap-6">
                                <div className="flex items-center gap-3 pr-4 border-r border-white/10">
                                    <span className={clsx("text-xs font-bold uppercase tracking-tight transition-colors", onlyShowUnassigned ? "text-orange-400" : "text-grey-medium")}>Masquer planifiés</span>
                                    <button 
                                        onClick={() => setOnlyShowUnassigned(!onlyShowUnassigned)} 
                                        title="Masquer les travaux déjà planifiés"
                                        className={clsx("w-12 h-6 rounded-full p-1 transition-all duration-300 relative", onlyShowUnassigned ? "bg-orange-500/50" : "bg-white/10")}
                                    >
                                        <div className={clsx("w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-300", onlyShowUnassigned ? "translate-x-6 bg-orange-400" : "translate-x-0")} />
                                    </button>
                                </div>
                                <button onClick={handleGoToWeekWork} className={clsx("flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all active:scale-95 shadow-lg", !allHomeworkAssigned && homeworkActivities.length > 0 ? "bg-white/10 text-grey-medium hover:bg-white/20 border border-white/10" : "bg-primary text-black hover:bg-primary/90 shadow-primary/20")}>
                        ...
                        {step === 'weekwork' && (
                            <button onClick={handleGoToSummary} className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm bg-primary text-black hover:bg-primary/90 transition-all active:scale-95 shadow-lg shadow-primary/20">
                        ...
                        {step === 'summary' && (
                            <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-black text-sm bg-emerald-500 text-white hover:bg-emerald-400 transition-all active:scale-95 uppercase tracking-wider shadow-xl shadow-emerald-500/20 disabled:opacity-50">
