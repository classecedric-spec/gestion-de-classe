 ...
 * Encrypt photo data
 */
const encryptPhoto = (base64Photo: string): string | null => {
    try {
        const key = getEncryptionKey();
        const encrypted = CryptoJS.AES.encrypt(base64Photo, key).toString();
 ...
 * Decrypt photo data
 */
const decryptPhoto = (encryptedPhoto: string): string | null => {
    try {
        const key = getEncryptionKey();
 ...
 */
export const getCachedPhoto = async (id: string, hash: string): Promise<string | null> => {
 ...
 */
export const setCachedPhoto = async (id: string, base64Photo: string, hash: string): Promise<void> => {
 ...
 */
export const clearPhotoCache = async (): Promise<boolean> => {
    try {
        const db = await initDB();
        const transaction = db.transaction([STORE_NAME], 'readwrite');
        const store = transaction.objectStore(STORE_NAME);
 ...
 * @param {string} id - Student or user ID
 */
export const deleteCachedPhoto = async (id: string): Promise<void> => {
    if (!id) return;
